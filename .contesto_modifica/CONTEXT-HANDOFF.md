...
UPDATE Product è stato provato manualmente e funziona.
...
È stato aggiunto un TODO in:

```text
FormHandler::handleFormCreate()
```

Il prossimo lavoro da fare è implementare CREATE.

Prima di scrivere il codice:

1. leggere il `FormHandler.php` reale nello ZIP;
2. verificare cosa restituisce realmente `dataHandler->create($data)`;
3. determinare in modo generico l'ID creato;
4. evitare di riutilizzare helper ExtraProperty soltanto per il logging;
5. scegliere il punto finale corretto del create;
6. loggare soltanto se l'intera operazione è riuscita;
7. verificare il record in `ps_log`.
