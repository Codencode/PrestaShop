...
final class NativeAdminSessionReaderTest extends TestCase
{
    public function testReadDoesNotWriteOrRenewAndPreservesFrontOfficeBags(): void
    {
        ...
        $request = Request::create('https://example.test/', 'GET', [], [session_name() => 'existing-session']);
        $reader = new NativeAdminSessionReader();
        self::assertSame($snapshot, $reader->read($request));
        self::assertSame($snapshot, $reader->read($request));
    ...
    public function testMissingAndInvalidCookieDoNotOpenSession(): void
    {
        ...
        $reader = new NativeAdminSessionReader();
        self::assertNull($reader->read(Request::create('https://example.test/')));
        self::assertNull($reader->read(Request::create('https://example.test/', 'GET', [], [session_name() => '../invalid'])));
    ...
    public function testUnknownSessionIsNotAcceptedOrWritten(): void
    {
        ...
        session_set_save_handler($handler, false);
        $reader = new NativeAdminSessionReader();
        $request = Request::create('https://example.test/', 'GET', [], [session_name() => 'unknown-session']);
    ...
    private function createHandler(string $data): SessionHandlerInterface&SessionUpdateTimestampHandlerInterface
    {
        return new class($data) implements SessionHandlerInterface, SessionUpdateTimestampHandlerInterface {
            public int $reads = 0;
            public int $writes = 0;
