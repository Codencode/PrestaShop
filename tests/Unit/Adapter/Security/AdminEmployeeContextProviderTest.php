...
final class AdminEmployeeContextProviderTest extends TestCase
{
    ...
    public function testValidation(string $scenario, bool $expectedValid): void
    {
        ...
        $employeeProvider = $this->createMock(EmployeeProvider::class);
        if ($scenario === 'deleted employee') {
            $employeeProvider->method('refreshUser')->willThrowException(new UserNotFoundException());
        } else {
            $employeeProvider->method('refreshUser')->willReturn($fresh);
        }
        ...
        $provider = new AdminEmployeeContextProvider($reader, $employeeProvider, $configuration);
        $request = Request::create('https://example.test/', 'GET', [], [], [], [
            'REMOTE_ADDR' => in_array($scenario, ['changed IP', 'IP check disabled'], true) ? '2001:db8::2' : '2001:db8::1',
        ]);
        if ($scenario === 'legacy cookie only') {
            $request->cookies->set('psAdmin', 'presence-is-not-authentication');
        }
        ...
        self::assertNotNull($provider->getContext($request));
        self::assertSame($serialized, $serverSession['_sf2_attributes']['_security_main']);
    }
